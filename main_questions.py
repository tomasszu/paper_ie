...
def main():
    ...
    user_prompt_temp = load_template(r"source\prompts\ie\user\user_prompt_questions.md")
    ...
    for file_path in input_path.iterdir():
        if file_path.is_file() and "methods" in file_path.name:
            ...
            user_prompt = user_prompt_temp.format(
                chapter_json = chunk_text, queried_fields_json = qf_json, context_json = context_json
            )
            ...
            if response.content != "None":
                ...
                output_file_path = Path(  
                    output_path,  
                    f"{file_path.name}_questions.json"
                )
